...
export default function TenantSettingsScreen() {
  ...
  const handleSwitchToLandlord = async () => {
    try {
      const { error } = await supabase.from('profiles').update({ role: 'landlord' }).eq('id', profile?.id);
      if (error) throw error;
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 20 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ marginTop: 8, gap: 12 }}>
          ...
          <Pressable style={styles.actionBtn} onPress={handleSwitchToLandlord}>
            <View style={[styles.iconBox, { backgroundColor: '#DBEAFE', opacity: 0.8 }]}>
              <Ionicons name="home" size={16} color="#2563EB" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[styles.actionBtnText, { color: '#2563EB' }]}>Switch to Landlord</Text>
              <Text style={styles.actionSubText}>Manage your own properties</Text>
            </View>
